...
In Claude Code, run `/plugin` and search for **firecrawl**, then select it to install. This installs both the skills and the bundled Firecrawl MCP server.

### 2. Sign In

The first time Claude uses a Firecrawl tool, you'll be prompted to authorize in your browser (OAuth). Approve it once — no API key to copy or paste. You can also check the connection any time with `/mcp`.
